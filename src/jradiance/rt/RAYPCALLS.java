...
//#ifndef RAYQLEN
public static final int RAYQLEN=		12;		/* # rays to send at once */
//#endif
...
//#else
public static final int MAX_NPROCS=	64;		/* max. # rendering processes */
//#endif
//#endif

//extern char	*shm_boundary;		/* boundary of shared memory */

static int		ray_pnprocs = 0;	/* number of child processes */
static int		ray_pnidle = 0;		/* number of idle children */

public class r_process {
//	int	pid;				/* child process id */
	Process	pid;				/* child process id */
	OutputStream	fd_send;			/* write to child here */
	InputStream	fd_recv;			/* read from child here */
	int	npending;			/* # rays in process */
	long[]	rno = new long[RAYQLEN];	/* working on these rays */
}			/* our child processes */
static r_process[] r_proc = new r_process[MAX_NPROCS];
static RAY[]	r_queue = new RAY[2*RAYQLEN];	/* ray i/o buffer */
...
boolean sendq_full()	{ return (r_send_next >= RAYQLEN); }
...
//	char[]	otnm,
//	int	nproc, OBJECT objsrc
//)
//{
//	if (objsrc.nobjects > 0)		/* close old calculation */
//		ray_pdone(0);
//
...
ray_pflush()			/* send queued rays to idle children */
{
	...
	for (i = ray_pnprocs; nc != 0 && i-- != 0; ) {
		...
		r_queue[sfirst].crtype = (short) n;
                // serialize n rays into a byte[]
                System.out.println("BROKEN CODE HERE\n");
//                byte[] rayBytes = 
//		nw = PROCESS.writebuf(r_proc[i].fd_send, r_queue[sfirst].getBytes()),
//				sizeof(RAY)*n);
//		if (nw != sizeof(RAY)*n)
//			return(-1);	/* write error */
		r_proc[i].npending = n;
		while (n-- != 0) {		/* record ray IDs */
...
//
//	if (poll==0)			/* count newly sent unless polling */
//		n = ray_pnprocs - ray_pnidle;
//	if (n <= 0)			/* return if nothing to await */
//		return(0);
//	if (poll==0 && ray_pnprocs == 1)	/* one process -> skip select() */
...
//		int	status;
//		ray_pnprocs--;
//		r_proc[ray_pnprocs].fd_send.close();
