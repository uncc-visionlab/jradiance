...
//#define MAXDST2		12
//				/* map a color */
    public static byte map_col(COLOR c, int p) {
        return clrmap[p][c.colval(p)<1. ? (int)(c.colval(p)*256.) : 255 ];
    }
...
static byte[][]	clrmap = new byte[3][256];
...
static void
make_gmap(			/* make gamma correction map */
double	gam)
{
	int	i;
	...
	for (i = 0; i < 256; i++)
		clrmap[COLOR.RED][i] =
		clrmap[COLOR.GRN][i] =
		clrmap[COLOR.BLU][i] = (byte) (256.0 * Math.pow((i+0.5)/256.0, 1.0/gam));
}
//
//
//void
...
//
public static void
map_color(		/* map a color to a byte triplet */
byte[]	rgb,
COLOR	col)
{  
	rgb[COLOR.RED] = map_col(col,COLOR.RED);
	rgb[COLOR.GRN] = map_col(col,COLOR.GRN);
	rgb[COLOR.BLU] = map_col(col,COLOR.BLU);
}
//
//
//static void
